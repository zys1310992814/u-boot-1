...
//------------------------------------------------------------------------------
static int MDIO_Write (unsigned long PhyReg, unsigned long Value)
{
...
//------------------------------------------------------------------------------
int MDIO_Read(unsigned long PhyReg, unsigned long *data) 
{
	unsigned long mst = 250;
	int sts = 0;
...
int HYPHYReset(void)
{
	...
	goodacc = MDIO_Write(PHY_REG_BMCR, PHY_BMCR_RESET_BIT);
	if (goodacc == 0) {
		return 0;
	}
	mst = 400;
...
int HWInit(bd_t * bd)
{
	...
	btemp = 0;
	while (mst > 0) {
		goodacc &= MDIO_Read(PHY_REG_BMSR, &tmp1);
...
int eth_init (bd_t * bd)
{
	...
	if ((e = getenv("macaddr")) != NULL) {
		...
		bd->bi_enetaddr[0] = (ep[0]<<4)|ep[1];
		bd->bi_enetaddr[1] = (ep[2]<<4)|ep[3];
		bd->bi_enetaddr[2] = (ep[4]<<4)|ep[5];
		bd->bi_enetaddr[3] = (ep[6]<<4)|ep[7];
		bd->bi_enetaddr[4] = (ep[8]<<4)|ep[9];
		bd->bi_enetaddr[5] = (ep[10]<<4)|ep[11];
		...
		bd->bi_enetaddr[2] = 0x90;
		bd->bi_enetaddr[3] = 0x00;
		bd->bi_enetaddr[4] = 0xC0;
		bd->bi_enetaddr[5] = 0x82;
		printf("Warning: No MAC in Env, Using "
			"%02X:%02X:%02X:%02X:%02X:%02X\n",
			bd->bi_enetaddr[0], bd->bi_enetaddr[1],
			bd->bi_enetaddr[2], bd->bi_enetaddr[3],
			bd->bi_enetaddr[4], bd->bi_enetaddr[5]);
